...
## EffectPatterns Subtree

The `resources/effect-patterns` directory contains a git subtree of the [EffectPatterns](https://github.com/PaulJPhilp/EffectPatterns) repository. This provides access to Effect-TS patterns and examples for reference when working with the library.

### Updating the Subtree

To update the EffectPatterns subtree to the latest version:

```bash
# Fetch the latest changes from the EffectPatterns repository
git fetch effect-patterns

# Pull the latest changes into the subtree
git subtree pull --prefix=resources/effect-patterns effect-patterns main --squash
```
...
# Pull a specific commit or tag
git subtree pull --prefix=resources/effect-patterns effect-patterns <commit-or-tag> --squash
```

### Git Remotes

The `effect-patterns` remote is configured to point to `git@github.com:PaulJPhilp/EffectPatterns.git` and is kept for easy updates. You can view all remotes with:
...
This subtree is intended for:
- Reference when working with Effect-TS patterns
- Understanding best practices and common patterns
- Providing context for LLMs working on this project
- Offline access to pattern examples and implementations
