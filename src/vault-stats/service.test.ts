...
describe("VaultStatsService", () => {
	it("should calculate metrics for empty cache", () =>
		Effect.gen(function* () {
			...
			Effect.provide(VaultCacheTest(new Map())),
		));
	...
	it("should calculate metrics for single file", () =>
		Effect.gen(function* () {
			...
			Effect.provide(VaultStatsService.Default),
			Effect.provide(
				VaultCacheTest(new Map([["test.md", "Line 1\nLine 2\nLine 3"]])),
			),
		));
	...
	it("should count bytes correctly using UTF-8 encoding", () =>
		Effect.gen(function* () {
			...
			Effect.provide(VaultStatsService.Default),
			Effect.provide(VaultCacheTest(new Map([["test.md", "Hello"]]))),
		));
});
