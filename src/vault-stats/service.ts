...
export class VaultStatsService extends Effect.Service<VaultStatsService>()(
	"VaultStatsService",
	{
				...
				getMetrics: (): Effect.Effect<VaultMetrics> =>
					Effect.gen(function* () {
						...
						return {
							...
							totalLines,
							averageFileSize:
								files.size > 0 ? Math.round(totalBytes / files.size) : 0,
							largestFile: largest.path ? largest : { path: "none", bytes: 0 },
							smallestFile: smallest.path
								? smallest
								: { path: "none", bytes: 0 },
						};
					}),
...
export const VaultStatsServiceTest = (
	getMetrics: () => Effect.Effect<VaultMetrics>,
