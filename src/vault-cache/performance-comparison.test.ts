...
const sequentialSearch = (cache: Map<string, string>, query: string): Array<any> => {
...
describe("Performance Comparison", () => {
  ...
  it("compares sequential vs concurrent performance", async () => {
    ...
    console.log(`Sequential: ${seqDuration.toFixed(2)}ms for ${testCache.size} files`)
    console.log(`Concurrent: ${concDuration.toFixed(2)}ms for ${testCache.size} files`)
    const speedup = seqDuration > concDuration ? seqDuration / concDuration : concDuration / seqDuration
