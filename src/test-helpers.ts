...
export const createTestVault = Effect.gen(function* () {
	...
	yield* Effect.promise(() =>
		fs.promises.writeFile(path.join(tempDir, "another.md"), "# Another Note\n\nDifferent content.")
	)

	const subDir = path.join(tempDir, "subfolder")
	yield* Effect.promise(() => fs.promises.mkdir(subDir))
	yield* Effect.promise(() =>
		fs.promises.writeFile(path.join(subDir, "nested-note.md"), "# Nested Note\n\nThis is nested in a subfolder.")
	)
 ...
 */
export const withTestVault = <A, E, R>(use: (vaultPath: string) => Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
	Effect.acquireUseRelease(createTestVault, use, cleanupTestVault)
